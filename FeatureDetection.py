...
def gass_filter(img,size,sigma):
      x,y= img.shape
      # r,g,b=cv2.split(img)
      # collored_img = [r,g,b]
      # i=np.floor(size-1/2)
...
def harris(k=0.04,thr=0.2):
    img = read_rgb(gray=False ,normalize = False)
    x,y,_=img.shape
    ...
    Ixx = gass_filter((Ix*Ix),3,1)
    Ixy = gass_filter((Iy*Ix),3,1)
    Iyy = gass_filter((Iy*Iy),3,1)
    ...
    R=deta-(k*trace*trace)

    img_cornor=np.copy(img)
    threshold=thr*(R.max())

    for i in range(x-2):
        for j in range(y-2):
            if R[i,j]>threshold:
                cv2.circle(img_cornor,(j,i),2,(0,0,255))
    fig = plt.figure()
    plt.imshow(img_cornor)
    plt.show
    path = saveImg_unique(img_cornor, "./static/img/output/", rgb=True, save_on_current = False)
    return path
########################### LAMBDA ####################################
def sobel_x(imggray):
    ##Sobel operator kernels.
...
# Compute the derivatives of the image intensity
# Ix=sobel_x(gray_img)
# Iy=sobel_y(gray_img)

    
def corners_with_lambda (threshold = 0.3):
    # Create a copy of the original image
   
    gray_img = read_rgb(gray=True ,normalize = False)
    ...
    M = np.zeros((gray_img.shape[0], gray_img.shape[1], 2, 2))
    M[:, :, 0, 0] = Ix * Ix
    M[:, :, 0, 1] = Ix * Iy
    M[:, :, 1, 0] = Ix * Iy
    M[:, :, 1, 1] = Iy * Iy
    ...
    # Extract the smaller eigenvalue lambda1
    lambda1 = eigenvalues[..., 0]
    # Apply thresholding to obtain a binary map of corners
    threshold_value = threshold * np.max(lambda1)
    corners = cv2.threshold(lambda1, threshold_value, 255, cv2.THRESH_BINARY)[1]
    # Dilate the corners to increase their area
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    corners = cv2.dilate(corners, kernel, iterations=2)

    # Replace the corresponding pixels in the image with green color
    img_with_corners[corners != 0] = [0, 255, 0]
    path = saveImg_unique(img_with_corners, "./static/img/output/", rgb=True, save_on_current = False)
    return path
...
def findScaleSpaceExtrema(gaussian_images, dog_images, num_intervals, sigma, image_border_width, contrast_threshold=0.04):
    ...
    """
    threshold = floor(0.5 * contrast_threshold /
                      num_intervals * 255)  # from OpenCV implementation
    keypoints = []
...
def draw_cross_keypoints(img, keypoints, color):
    ...
    for kp in keypoints:
        x, y = kp.pt  # Each keypoint as an x, y tuple

        x = int(round(x))  # Round and cast to int
        y = int(round(y))

        # Draw a cross with (x, y) center
        cv2.drawMarker(img_kp, (x, y), color, markerType=cv2.MARKER_CROSS,
                       markerSize=5, thickness=1, line_type=cv2.LINE_8)
...
def SIFT():
    img_gray = read_rgb(gray=True,normalize=False)
    img_rgb =  read_rgb(gray=False,normalize=False)
    (keypoints, descriptors) = computeKeypointsAndDescriptors(img_gray)


    img_kp = draw_cross_keypoints(img_rgb, keypoints, color=(255, 0, 0))  # Draw keypoints as "+" signs
    path = saveImg_unique(img_kp, "./static/img/output/", rgb=True, save_on_current = False)
    print(path)
...
def SIFT_2 (img_path):
    img_gray = img1 = cv2.imread(img_path, 0)
    img_rgb =  img1 = cv2.imread(img_path)
    (keypoints, descriptors) = computeKeypointsAndDescriptors(img_gray)
    return (keypoints, descriptors)
